...
fn backup(diff_list: &[String], root_path: &Path, dest_path: &Path) -> Result<()> {
    ...
    for source_path in diff_list {
        ...
        let parent = dest_path_abs.parent().unwrap();
...
fn ensure_backup_dir(dir_path: &Path) -> Result<()> {
    match dir_path.metadata() {
        ...
        Err(err) => {
            ...
            // We can unwrap here because earlier dest_path was already checked to exist,
            // so there will always be at least that as a parent.
            let parent = dir_path.parent().unwrap();
            ensure_backup_dir(parent)?;

            fs::create_dir(dir_path).with_context(|| {
                format!("cannot create directory '{}': {}", dir_path.display(), err)
            })
            // fs::set_permissions() // TODO
        }
    }
}
